...
from pyspark.sql.functions import window, explode, split, from_json, max, min, sum, avg
...
def consume_kafka_events(kafka_server):
    ...
    df = windowed_df.select(
        "sensor_id",  "min_temperature", "sum_temperature", "avg_temperature", "max_temperature"
    )
