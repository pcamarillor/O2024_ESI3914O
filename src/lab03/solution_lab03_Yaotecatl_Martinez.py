...
def get_total_passengers(flights_df):
    ...
    '''
    return flights_df.groupBy("airline").sum("passengers").withColumnRenamed("sum(passengers)", "total_passengers")
...
def get_avg_passengers_per_flight_per_destination_country(flights_df):
    ...
    return flights_df.groupBy("DestinationCountry").agg((sum(flights_df["Passengers"]) / count(flights_df["FlightNumber"])).alias('avg(Passengers)'))
